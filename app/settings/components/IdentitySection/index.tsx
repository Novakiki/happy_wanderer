...
export type IdentityAuthorPreference = {
  contributor_id: string | null;
...
export default function IdentitySection({ profileRelation }: Props) {
  ...
  const loadIdentity = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : 'Failed to load identity settings.';
      setError(message);
    } finally {
      if (!silent) {
        setLoading(false);
  ...
  const handleDefaultVisibility = async (value: Visibility) => {
    ...
    try {
      ...
      setSaved('Default visibility updated.');
      await loadIdentity({ silent: true });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to update default visibility.';
      setError(message);
    } finally {
      setSaving(false);
    }
  ...
  const handleNoteVisibility = async (referenceId: string, value: Visibility) => {
    ...
    try {
      ...
      setSaved('Note visibility updated.');
      await loadIdentity({ silent: true });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to update note visibility.';
      setError(message);
    } finally {
  ...
  const handleAuthorVisibility = async (contributorId: string, value: Visibility) => {
    ...
    try {
      ...
      setSaved('Author preference updated.');
      await loadIdentity({ silent: true });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to update author preference.';
      setError(message);
    } finally {
  ...
  const handleBulkAuthorVisibility = async (contributorId: string, value: Visibility) => {
    ...
    try {
      ...
      setSaved(`Updated ${authorNotes.length} notes.`);
      await loadIdentity({ silent: true });
    } catch (err) {
  ...
  const handleClaimIdentity = async () => {
    ...
    try {
      ...
      setSaved('Identity claimed.');
      await loadIdentity({ silent: true });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to claim identity.';
      setError(message);
    } finally {
  ...
  return (
    <div className="p-5 rounded-xl border border-white/10 bg-white/[0.02]">
      ...
      {loading ? (
        ...
        <p className={`${formStyles.error} mt-3`}>{error}</p>
      ) : !identity?.person ? (
